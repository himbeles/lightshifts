...
def _tex_state(state, F=None, show_term=True):
    ...
    return (
        term_str
        + r"${}^{%s}\mathrm{%s}_{%s}$" % (config[0], config[1], config[2])
        + Fstr
    )


def plot_total_lightshift_around_hyperfine_state(
    atom_filename,
    ...
    for mf in possible_mf:
        ls_data.append(
            smart_gen_array(
                lambda w: ls.total_lightshift(w, q, mf), lam_0, lam_1, sing, n=n
            )
        )

    # plot title
    title = {-1: "$\sigma^-$ light", 1: "$\sigma^+$ light", 0: "$\pi$ light"}
    # legend title
    leg_title = {-1: "$m_F$", 1: "$m_F$", 0: "$|m_F|$"}
    ...
    for i, mf in enumerate(possible_mf):
        ...
        plt.plot(
            ...
            l,
            c=m.to_rgba(mf),
            label=r"{:+1.0f}/2".format(mf * 2),
        )

    for w in sing:
        plt.plot(
            [(c / w - fzero) / 1e9, (c / w - fzero) / 1e9],
            [-1e6, 1e6],
            ":",
            color="gray",
            label="",
    ...
    plt.xlabel(
        r"detuning from %s $\rightarrow$ %s (GHz)"
        % (_tex_state(ls.state_i), _tex_state(state_f, Ff))
    )
...
def plot_scalar_lightshift(
    atom_filename, transitions_filename, lam_min=200e-9, lam_max=1500e-9, n=200
):
    from lightshifts import LightshiftSolver
