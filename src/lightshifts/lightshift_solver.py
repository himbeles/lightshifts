...
class LightshiftSolver:
    def __init__(self, atom_filename, transitions_filename, Fi=None):
        ...
        """
        self.atom, self.transitions = self._import_states_and_transitions(
            atom_filename, transitions_filename
        )
                ...
                d.append(
                    {
                        ...
                        "_ref_frequency": self._query_state_prop(
                            state_u, "_ref_frequency"
                        ),
    ...
    def pretty_transition_table(self, latex=False):
        ...
        trans[r"$\lambda$ (nm)"] = trans["wavelength"].apply(
            lambda x: "{:1.3f}".format(x * 1e9)
        )
        trans[r"k (1/cm)"] = trans["wavenumber"].apply(lambda x: "{:1.2f}".format(x))
        trans[r"$\Gamma$ (MHz)"] = trans["Gamma"].apply(
            lambda x: "{:1.2f}".format(x / 1e6)
        )
        trans[r"$\tau_f$ (ns)"] = trans["lifetime_f"].apply(
            lambda x: "{:1.3f}".format(x * 1e9)
        )
        trans["br.r. (LS)"] = trans["branching_ratio_LS"].apply(
            lambda x: "n/a" if np.isnan(x) else "{:1.4f}".format(x)
        )
        trans[r"ref. ($\tau_f$, $\Gamma$)"] = trans["_ref_Gamma"].apply(
            lambda x: "{:s}".format(x)
        )
        trans_export = trans[
            [
    ...
    def finestruct_reduced_mat_el_sqd(cls, Ji, Jf, omega_Jf_Ji, Gamma):
        ...
        if omega_Jf_Ji > 0:
            ...
            conj_ratio = (2 * Jg + 1) / (
                2 * Je + 1
            )  # see Steck 7.250, conj. red. mat. el.
    ...
    def hyperfine_reduced_mat_el_sqd(cls, I, Fi, Ff, Ji, Jf, omega_Jf_Ji, Gamma):
        ...
        y = (2 * Ji + 1) * (2 * Ff + 1)
        z = (
            float(wigner_6j(float(Ji), float(Jf), 1, float(Ff), float(Fi), float(I)))
        ) ** 2
        return fma * y * z
        ...
        for trans in self.transitions:
            ...
            for Ff in self._calc_Fs(Jf):
                a = (
                    2
                    * omega_Ff_Fi
                    * self.hyperfine_reduced_mat_el_sqd(
    ...
    def polarizabilities(self, lam):
        ...
        for trans in self.transitions:
            ...
            for Ff in self._calc_Fs(Jf):
                omega_Ff_Fi = (
                    2 * np.pi * self.transition_frequency_hyperfine(state_f, Ff)
                )
                ...
                p1 += (
                    (-1) ** (Fi + Ff + 1)
                    * np.sqrt(6 * Fi * (2 * Fi + 1) / (Fi + 1))
                    * float(wigner_6j(1, 1, 1, float(Fi), float(Fi), float(Ff)))
                    * f
                    * (omega / omega_Ff_Fi)
                )
                p2 += (
                    ...
                    * float(wigner_6j(1, 1, 2, float(Fi), float(Fi), float(Ff)))
                    * f
                )

        return p0, p1, p2

    def lightshifts(self, lam, q, mFi, laser_intensity=1.0):
        """
    ...
    def total_lightshift(self, lam, q, mFi, laser_intensity=1.0):
        """
